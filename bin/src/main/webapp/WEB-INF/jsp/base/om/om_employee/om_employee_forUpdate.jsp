	...
	<form id="empForm" name="empForm" action="${ctx }/omEmployee/update.do" method="post">
		...
		<table class="form_table">
            ...
            <tr>
                ...
                <td>
					<select class="formselect" name="CARD_TYPE" id="CARD_TYPE" >
						<%-- <c:forEach items='${dictList}' var='dict'>
							<c:if test="${dict.dictcode =='DOCUMENTTYPE' || dict.dictcode ==''}">
								<option value='${dict.value}' ${record.cardtype==dict.value?'selected':''}>${dict.name}</option>
							</c:if>
						</c:forEach> --%>
						<option value="" >请选择...      </option>
						<option value='1' ${record.CARD_TYPE==1?'selected':'' }>身份证</option>
					</select>
                </td>
...
<%--             <tr>
                <th>证件类型:</th>
                <td><input type="text" class="forminput" id="CARD_TYPE" name="CARD_TYPE" value="${record.CARD_TYPE}"/></td>
                <th>证件号码:</th>
                <td><input type="text" class="forminput" id="CARD_NO" name="CARD_NO" value="${record.CARD_NO}"/></td>
            ...
            <tr>
                ...
                <th>可管理机构:</th>
                <td>
                	<input type="text" class="forminput" id="ORG_ID_LIST" name="ORG_ID_LIST" value="${record.ORG_ID_LIST}"/>
                </td>
            </tr>
			<tr>
				<%-- <th>主岗位:</th>
