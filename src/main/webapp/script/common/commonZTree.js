...
//初始化左边树形菜单;
function loadTreeWithParam(url,param,type){
	$.ajax({
		type: "post",
		url: url,
		dataType:"json",
		...
		success: function(data) {
			...
			var setting = checkTreeSetting;
			if(type=='CHECK'){
				
			}else if(type=='RADIO'){
				setting = radioTreeSetting;
			...
			mainTreeObj = $.fn.zTree.getZTreeObj("zTree");
		}
	});
}
